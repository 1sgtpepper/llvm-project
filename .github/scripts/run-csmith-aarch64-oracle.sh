...
cp "$CSMITH_ROOT/provenance.txt" "$RESULT_ROOT/csmith-provenance.txt"
ulimit -c 0
...
compile_native() {
  ...
  local level=$2
  local stem=$3
  timeout 45 "$compiler" -std=c11 -w "$level" \
    -I "$CSMITH_INCLUDE" "$RESULT_ROOT/work/test.c" \
...
compile_cross_gcc() {
  local level=$1
  local stem=$2
  timeout 45 "$CROSS_GCC" -std=c11 -w "$level" -static \
    -I "$CSMITH_INCLUDE" "$RESULT_ROOT/work/test.c" \
    -o "$RESULT_ROOT/work/$stem" \
    >"$RESULT_ROOT/work/$stem.compile" 2>&1
}
...
compile_cross_clang() {
  local level=$1
  local stem=$2
  if ! timeout 45 "$CLANG" --target=aarch64-linux-gnu \
      --gcc-toolchain=/usr -std=c11 -w "$level" \
      -I "$CSMITH_INCLUDE" -c "$RESULT_ROOT/work/test.c" \
      -o "$RESULT_ROOT/work/$stem.o" \
      >"$RESULT_ROOT/work/$stem.compile" 2>&1; then
    return 1
  fi
  if ! timeout 20 "$CROSS_GCC" -static "$RESULT_ROOT/work/$stem.o" \
    -o "$RESULT_ROOT/work/$stem" \
...
preserve_candidate() {
  ...
  timeout 45 "$CLANG" --target=aarch64-linux-gnu --gcc-toolchain=/usr \
    -std=c11 -w -O0 -I "$CSMITH_INCLUDE" -S -emit-llvm \
    "$RESULT_ROOT/work/test.c" -o "$RESULT_ROOT/candidate/clang-o0.ll" \
    >"$RESULT_ROOT/candidate/clang-o0-ir.log" 2>&1 || true
  timeout 45 "$CLANG" --target=aarch64-linux-gnu --gcc-toolchain=/usr \
    -std=c11 -w -O2 -I "$CSMITH_INCLUDE" -S -emit-llvm \
    "$RESULT_ROOT/work/test.c" -o "$RESULT_ROOT/candidate/clang-o2.ll" \
    >"$RESULT_ROOT/candidate/clang-o2-ir.log" 2>&1 || true
  timeout 45 "$CLANG" --target=aarch64-linux-gnu --gcc-toolchain=/usr \
    -std=c11 -w -O0 -I "$CSMITH_INCLUDE" -S \
    "$RESULT_ROOT/work/test.c" -o "$RESULT_ROOT/candidate/clang-o0.s" \
    >"$RESULT_ROOT/candidate/clang-o0-asm.log" 2>&1 || true
  timeout 45 "$CLANG" --target=aarch64-linux-gnu --gcc-toolchain=/usr \
    -std=c11 -w -O2 -I "$CSMITH_INCLUDE" -S \
    "$RESULT_ROOT/work/test.c" -o "$RESULT_ROOT/candidate/clang-o2.s" \
    >"$RESULT_ROOT/candidate/clang-o2-asm.log" 2>&1 || true
  timeout 45 "$CROSS_GCC" -std=c11 -w -O2 -I "$CSMITH_INCLUDE" -S \
    "$RESULT_ROOT/work/test.c" -o "$RESULT_ROOT/candidate/gcc-aarch64-o2.s" \
    >"$RESULT_ROOT/candidate/gcc-aarch64-o2-asm.log" 2>&1 || true
...
for ((offset = 0; offset < CASE_COUNT; offset++)); do
  ...
  source_bytes=$(wc -c <"$RESULT_ROOT/work/test.c")
  if [[ "$source_bytes" -gt 2000000 ]]; then
    oversize_count=$((oversize_count + 1))
    continue
  fi

  if ! compile_native "$CLANG" -O0 native-clang-o0 || \
      ! compile_native "$CLANG" -O2 native-clang-o2 || \
      ! compile_native gcc -O0 native-gcc-o0 || \
      ! compile_native gcc -O2 native-gcc-o2; then
    native_compile_reject_count=$((native_compile_reject_count + 1))
    continue
  fi

  if ! execute_native native-clang-o0 || \
      ! execute_native native-clang-o2 || \
  ...
  if ! same_output native-clang-o0 native-clang-o2 || \
      ! same_output native-clang-o0 native-gcc-o0 || \
      ! same_output native-clang-o0 native-gcc-o2; then
  ...
  if ! same_output native-clang-o0 aarch64-gcc-o0 || \
      ! same_output native-clang-o0 aarch64-gcc-o2; then
...
native_gcc_version=$(gcc --version | head -n1)
cross_gcc_version=$($CROSS_GCC --version | head -n1)
qemu_version=$($QEMU --version | head -n1)
csmith_binary_sha256=$(sha256sum "$CSMITH" | cut -d' ' -f1)
EOF

rm -rf "$RESULT_ROOT/work"
printf '%s\n' "$status"
