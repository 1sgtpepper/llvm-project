...
for ((offset = 0; offset < CASE_COUNT; offset++)); do
  ...
  timeout 15 "$OPT" '-passes=default<O2>' -verify-each -disable-output \
    "$RESULT_ROOT/work/test.ll" >"$RESULT_ROOT/work/opt.log" 2>&1
  opt_status=$?
  if [[ "$opt_status" -eq 0 ]]; then
    continue
  elif [[ "$opt_status" -eq 124 ]]; then
    status=opt-timeout
  else
    status=opt-failure
  fi
  ...
  cp "$RESULT_ROOT/work/opt.log" "$RESULT_ROOT/candidate/opt.log"
  break
done
...
opt_version=$(head -n 1 "$LLVM_ROOT/clang-version.txt")
EOF
