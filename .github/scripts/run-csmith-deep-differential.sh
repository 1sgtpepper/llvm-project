...
for ((offset = 0; offset < CASE_COUNT; offset++)); do
  ...
  if ! timeout 20 "$CSMITH" --seed "$seed" --no-packed-struct \
      --no-volatiles --max-funcs 10 --max-block-depth 8 \
      --max-expr-complexity 50 --output "$RESULT_ROOT/work/test.c"; then
  ...
  compile "$CLANG" -O2 "$RESULT_ROOT/work/clang-o2" \
    "$RESULT_ROOT/work/clang-o2.compile"
  o2_compile_status=$?
